...
| 区域         | 作用                                              | 共享性   | 存储内容                                   |
| ------------ | ------------------------------------------------- | -------- | ------------------------------------------ |
| 堆内存       | 存放对象实例,可以细分为新生代和老年代             | 线程共享 | new出来的对象(属性,方法的地址(指向方法区)) |
| 方法区       | 内有运行时常量池,也有人称之为永久代                     | 线程共享 | 常量,static变量,类信息(属性,方法) |
| 运行时常量池 |方法区的一部分| 线程共享 | 各种字面量和符号引用 |
| 程序计数器   | 比较小的内存区域,指示当前线程所执行的字节码的位置 | 线程隔离 |                                            |
| VM栈        | 记录方法调用 | 线程隔离 |局部变量表,对象的引用指针|
| 本地方法栈   | 执行Native方法时使用                                | 线程隔离 |                                            |
...
| :-------- | :------------------------------------------------------------ |
| jps      | 查询正在运行的JVM进程                                        |
| jstat    | 实时显示本地或远程JVM进程中类装载、内存、垃圾收集、JIT编译等数据 |
| jinfo    | 查询当前运行着的JVM属性和参数的值                            |
| jmap     | 显示当前Java堆和永久代的详细信息                             |
| jhat     | 分析使用jmap生成的dump文件                                   |
| jstack   | 生成当前JVM的所有线程快照,线程快照是虚拟机每一条线程正在执行的方法,目的是定位线程出现长时间停顿的原因. |
| jconsole | jvm控制台,图像化显示堆栈等使用情况,可以手动进行GC,非常实用   |
| jcmd     |                                                              |
...
1. [jvm系列(二):JVM内存结构](http://www.ityouknow.com/jvm/2017/08/25/jvm-memory-structure.html)
