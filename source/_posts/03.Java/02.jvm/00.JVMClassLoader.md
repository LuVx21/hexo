...
* 指的是将类 Class 文件读入内存，并为之创建一个 java.lang.Class 对象, class 文件被载入到了内存之后，才能被其它 class 所引用
...
- 启动类加载器(bootstrapClassLoader):加载Java核心库(JAVA_HOME/lib)及`-Xbootclasspath`配置,如`rt.jar`,`resources.jar`,`charsets.jar`,
